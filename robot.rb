class Robot
	...
	def move(position)
		if @orientation == S
			@position y == 0 ? if true puts "I don't want to fal off the edge." return 
			@position y == y - 1 
		elsif @orientation == N
			@position y == 5 ? if true puts "I don't want to fal off the edge." return
			@position y == y + 1
		elsif @orientation == E 
			@position x == 5 ? if true puts "I don't want to fal off the edge." return
			@position x == x + 5
		else
			@position x == 0 ? if true puts "I don't want to fal off the edge." return
			@position x == x - 1
		end						
	end
